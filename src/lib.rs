...
impl Driver {
    ...
    pub fn init(&mut self, config: &DriverConfig) -> Result<()> {
        let success = unsafe {
            self.inner.pin_mut().init(
                config.raw.as_ref().ok_or_else(|| DriverError::new("invalid config"))?,
            )
        };
        if success {
            Ok(())
    ...
    pub fn start(&mut self) -> Result<()> {
        if unsafe { self.inner.pin_mut().start() } {
            Ok(())
        } else {
    ...
    pub fn get_temperature(&mut self) -> Result<f32> {
        let mut temp = 0.0f32;
        if unsafe { self.inner.pin_mut().get_temperature(Pin::new_unchecked(&mut temp)) } {
            Ok(temp)
        } else {
    ...
    pub fn get_device_info(&mut self) -> Result<DeviceInfo> {
        let mut info = unsafe {
            let mut uninit: std::mem::MaybeUninit<DeviceInfo> = std::mem::MaybeUninit::uninit();
            uninit.assume_init()
        };
        if unsafe { self.inner.pin_mut().get_device_info(Pin::new_unchecked(&mut info)) } {
            Ok(info)
        } else {
    ...
    pub fn get_device_status(&mut self) -> Result<DeviceStatus> {
        let mut status = unsafe {
            let mut uninit: std::mem::MaybeUninit<DeviceStatus> = std::mem::MaybeUninit::uninit();
            uninit.assume_init()
        };
        if unsafe { self.inner.pin_mut().get_device_status(Pin::new_unchecked(&mut status)) } {
            Ok(status)
        } else {
...
impl DriverConfig {
    ...
    pub fn lidar_type(&mut self, ty: LidarType) -> &mut Self {
        unsafe { carvi_rsd::driver_param_set_lidar_type(self.raw.pin_mut(), ty) };
        self
    }

    pub fn input_type(&mut self, ty: InputType) -> &mut Self {
        unsafe { carvi_rsd::driver_param_set_input_type(self.raw.pin_mut(), ty) };
        self
    }

    pub fn frame_id(&mut self, id: &str) -> &mut Self {
        unsafe { carvi_rsd::driver_param_set_frame_id(self.raw.pin_mut(), id) };
        self
    }

    pub fn input<'a>(&'a mut self) -> DriverInput<'a> {
        let pin = unsafe { carvi_rsd::driver_param_input(self.raw.pin_mut()) };
        DriverInput { raw: pin }
    }

    pub fn decoder<'a>(&'a mut self) -> DriverDecoder<'a> {
        let pin = unsafe { carvi_rsd::driver_param_decoder(self.raw.pin_mut()) };
...
impl<'a> DriverInput<'a> {
    pub fn host_address(&mut self, addr: &str) {
        unsafe { carvi_rsd::input_param_set_host_address(self.raw.as_mut(), addr) };
    }

    pub fn group_address(&mut self, addr: &str) {
        unsafe { carvi_rsd::input_param_set_group_address(self.raw.as_mut(), addr) };
    }

    pub fn msop_port(&mut self, port: u16) {
        unsafe { carvi_rsd::input_param_set_msop_port(self.raw.as_mut(), port) };
    }

    pub fn difop_port(&mut self, port: u16) {
        unsafe { carvi_rsd::input_param_set_difop_port(self.raw.as_mut(), port) };
    }

    pub fn imu_port(&mut self, port: u16) {
        unsafe { carvi_rsd::input_param_set_imu_port(self.raw.as_mut(), port) };
    }

    pub fn user_layer_bytes(&mut self, bytes: u16) {
        unsafe { carvi_rsd::input_param_set_user_layer_bytes(self.raw.as_mut(), bytes) };
    }

    pub fn tail_layer_bytes(&mut self, bytes: u16) {
        unsafe { carvi_rsd::input_param_set_tail_layer_bytes(self.raw.as_mut(), bytes) };
    }

    pub fn socket_recv_buf(&mut self, bytes: u32) {
        unsafe { carvi_rsd::input_param_set_socket_recv_buf(self.raw.as_mut(), bytes) };
    }

    pub fn pcap_path(&mut self, path: &str) {
        unsafe { carvi_rsd::input_param_set_pcap_path(self.raw.as_mut(), path) };
    }

    pub fn pcap_repeat(&mut self, repeat: bool) {
        unsafe { carvi_rsd::input_param_set_pcap_repeat(self.raw.as_mut(), repeat) };
    }

    pub fn pcap_rate(&mut self, rate: f32) {
        unsafe { carvi_rsd::input_param_set_pcap_rate(self.raw.as_mut(), rate) };
    }

    pub fn use_vlan(&mut self, enabled: bool) {
        unsafe { carvi_rsd::input_param_set_use_vlan(self.raw.as_mut(), enabled) };
    }
}
...
impl<'a> DriverDecoder<'a> {
    pub fn min_distance(&mut self, value: f32) {
        unsafe { carvi_rsd::decoder_param_set_min_distance(self.raw.as_mut(), value) };
    }

    pub fn max_distance(&mut self, value: f32) {
        unsafe { carvi_rsd::decoder_param_set_max_distance(self.raw.as_mut(), value) };
    }

    pub fn use_lidar_clock(&mut self, enabled: bool) {
        unsafe { carvi_rsd::decoder_param_set_use_lidar_clock(self.raw.as_mut(), enabled) };
    }

    pub fn dense_points(&mut self, enabled: bool) {
        unsafe { carvi_rsd::decoder_param_set_dense_points(self.raw.as_mut(), enabled) };
    }

    pub fn timestamp_first_point(&mut self, enabled: bool) {
        unsafe { carvi_rsd::decoder_param_set_ts_first_point(self.raw.as_mut(), enabled) };
    }

    pub fn wait_for_difop(&mut self, enabled: bool) {
        unsafe { carvi_rsd::decoder_param_set_wait_for_difop(self.raw.as_mut(), enabled) };
    }

    pub fn config_from_file(&mut self, enabled: bool) {
        unsafe { carvi_rsd::decoder_param_set_config_from_file(self.raw.as_mut(), enabled) };
    }

    pub fn angle_path(&mut self, path: &str) {
        unsafe { carvi_rsd::decoder_param_set_angle_path(self.raw.as_mut(), path) };
    }

    pub fn start_angle(&mut self, angle: f32) {
        unsafe { carvi_rsd::decoder_param_set_start_angle(self.raw.as_mut(), angle) };
    }

    pub fn end_angle(&mut self, angle: f32) {
        unsafe { carvi_rsd::decoder_param_set_end_angle(self.raw.as_mut(), angle) };
    }

    pub fn split_frame_mode(&mut self, mode: SplitFrameMode) {
        unsafe { carvi_rsd::decoder_param_set_split_frame_mode(self.raw.as_mut(), mode) };
    }

    pub fn split_angle(&mut self, angle: f32) {
        unsafe { carvi_rsd::decoder_param_set_split_angle(self.raw.as_mut(), angle) };
    }

    pub fn num_blocks(&mut self, count: u16) {
        unsafe { carvi_rsd::decoder_param_set_num_blks_split(self.raw.as_mut(), count) };
    }

    pub fn transform(&mut self) -> TransformParams<'_> {
        let pin = unsafe { carvi_rsd::decoder_param_transform(self.raw.as_mut()) };
...
impl<'a> TransformParams<'a> {
    pub fn x(&mut self, value: f32) {
        unsafe { carvi_rsd::transform_param_set_x(self.raw.as_mut(), value) };
    }
    pub fn y(&mut self, value: f32) {
        unsafe { carvi_rsd::transform_param_set_y(self.raw.as_mut(), value) };
    }
    pub fn z(&mut self, value: f32) {
        unsafe { carvi_rsd::transform_param_set_z(self.raw.as_mut(), value) };
    }
    pub fn roll(&mut self, value: f32) {
        unsafe { carvi_rsd::transform_param_set_roll(self.raw.as_mut(), value) };
    }
    pub fn pitch(&mut self, value: f32) {
        unsafe { carvi_rsd::transform_param_set_pitch(self.raw.as_mut(), value) };
    }
    pub fn yaw(&mut self, value: f32) {
        unsafe { carvi_rsd::transform_param_set_yaw(self.raw.as_mut(), value) };
    }
}
